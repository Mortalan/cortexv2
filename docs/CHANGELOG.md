...
- **SSL Termination for GLPI & Auth Subdomains:** Successfully requested and issued a new Let's Encrypt certificate covering all 10 target domains (including `auth.rmmservice.co.za` and `glpi.rmmservice.co.za`). Compiled the new PEM bundle `/etc/haproxy/rmmservice.co.za.pem` and reloaded HAProxy, completely resolving the browser "not secure" warning on GLPI.
