...
docker exec velociraptor ./velociraptor --config /velociraptor/server.config.yaml artifacts collect --client_id "$CLIENT_ID" Windows.Remediation.Quarantine
...
if [ $? -eq 0 ]; then
    ...
    docker exec velociraptor ./velociraptor --config /velociraptor/server.config.yaml artifacts collect --client_id "$CLIENT_ID" Linux.Remediation.Quarantine
fi
